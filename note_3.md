...
程序在其运行时间内可能表现出截然不同的行为，这些行为即使在大尺度上也能观察到。理解这些大规模的程序行为可以解锁许多新的优化方法。这些优化方法包括：利用线程行为变化信息的新线程调度算法、不仅针对代码整体性能而且针对执行各个阶段进行反馈导向优化的方法，以及能够准确模拟整个程序行为的仿真技术。为了实现这些优化，我们首先需要开发能够自动且高效地分析程序在大段执行过程中的行为的分析工具。
